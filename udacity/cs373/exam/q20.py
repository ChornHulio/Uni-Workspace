...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    h = calc_h(1./road[init[0]][init[1]],road,init,goal,lane_change_cost)
    queue = add_to_queue([],h,1./road[init[0]][init[1]],init)
    while queue:
        item = queue.pop(0)
        cost = item[1]
        pos = item[2]
        if pos == goal:
            return cost
        queue = expand(queue,cost,pos,road,lane_change_cost,goal)
    print "error 1"
    return None

def calc_h(cost,road,pos,goal,lcc):
    h = cost
    h += abs(goal[0]-pos[0])*lcc
    # for all steps right
    for step in range(pos[1],len(road[0])):
        # search for min lane cost
        min_cost = 1. / 100000
        for lane in range(pos[0],len(road)-1):
            if road[lane][step] > min_cost :
                min_cost = road[lane][step]
        h += 1. / min_cost
    return h        

def add_to_queue(queue,h,cost,pos):
    queue.append([h,cost,pos])
    queue.sort()
    return queue

def expand(queue,cost,pos,road,lcc,goal):
    if pos[1]+1 >= len(road[0]): #too far east
        return queue

    for i in [-1,0,1]:
        posN = [pos[0]+i,pos[1]+1]
        if posN[0] >= 0 and posN[0] < len(road):
            if road[posN[0]][posN[1]] > 0:
                costN = cost
                if i != 0:
                    costN += lcc
                h = costN
                if posN != goal:
                    costN += 1./road[posN[0]][posN[1]]
                    h = calc_h(costN,road,posN,goal,lcc)
                queue = add_to_queue(queue,h,costN,posN)
    return queue
