$(function () {
    ...
    var width = 800,
        ...
        txt = {"font": '14px Fontin-Sans, Arial', stroke: "none", fill: "#000"},
    ...
    for (var i = 0, ii = axisy.length; i < ii; i++) {
        for (var j = 0, jj = axisx.length; j < jj; j++) {
            var R = data[o] && Math.min(Math.round(Math.sqrt(data[o] / Math.PI) * 3.2), max);
            if (R) {
                (function (dx, dy, R, value) {
                    var color = "hsb(" + [0,0,.3] + ")"; // "hsb(" + [(1 - R / max) * .5, 1, .75] + ")";
